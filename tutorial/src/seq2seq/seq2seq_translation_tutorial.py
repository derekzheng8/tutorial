...
class AttnDecoderRNN(nn.Module):
    def __init__(self,hidden_size,output_size,dropout_p,max_length=MAX_LENGTH):
        ...
        self.dropout = nn.Dropout(self.dropout_p)
        self.softmax = nn.LogSoftmax(dim=1)
        self.atten = nn.Linear(hidden_size*2,self.max_length)
    ...
    def forward(self, input,hidden,encoder_outputs):
        ...
        output = self.i2h(output).unsqueeze(0)
        output = F.relu(output)
        output,hidden = self.gru(output,hidden)
        output = F.log_softmax(self.out(output[0]),dim=1)
...
gene_decoder = DecoderRNN(hidden_size,output_lang.n_words).to(device)
trainIters(encoder1, attn_decoder1, 75000, print_every=5000)
#trainIters(encoder1, gene_decoder, 7500, print_every=5000)
######################################################################
#
